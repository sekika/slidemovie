...
*   プロンプトは **各チャンクに再適用** されるため、ナレーション全体で声色・口調が一貫します。
*   `chunk_size` は **読み上げ本文の長さのみ** で評価され、プロンプト長は含まれません。

> 旧バージョンでは分割前にプロンプトを本文へ前置きしていたため、スタイル指示が先頭チャンクにしか乗りませんでした。この制約は解消され、プロンプトはすべてのチャンクに適用されます。
